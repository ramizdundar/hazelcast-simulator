/*
 * Copyright (c) 2008-present, Hazelcast, Inc. All Rights Reserved.
 *
 ...
 */
package com.hazelcast.simulator.tests.platform;
...
public class IntByteMapWithJetTest extends HazelcastTest {
    ...
    public int maxIdleSeconds = 10;
    public int eventPerSecond = 1_000;
    ...
    public void prepareJetJob() {
        ...
        logger.info("Event per second: " + eventPerSecond);
        p.readFrom(TestSources.longStream(eventPerSecond, 100))
                .withoutTimestamps()
                .writeTo(Sinks.noop());
        job = targetInstance.getJetInstance().newJob(p);
    }
    ...
    @TimeStep(prob = 0.0)
    public void setWithTtl(ThreadState state) {
        map.set(state.randomWriteKey(), state.randomValue(), state.randomTtl(), TimeUnit.SECONDS);
    }

    @TimeStep(prob = 0.0)
    public void setWithMaxIdle(ThreadState state) {
        map.set(state.randomWriteKey(), state.randomValue(), state.randomMaxIdle(), TimeUnit.SECONDS);
    }
    ...
    public class ThreadState extends BaseThreadState {
        ...
        private int randomTtl() {
            return randomInt(ttlSeconds);
        }

        private int randomMaxIdle() {
            return randomInt(maxIdleSeconds);
        }
    }
